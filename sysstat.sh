...
HD=$(df -hl /dev/vda2 | grep vda2 | awk '{printf ("%02d", $5)}' | sed -e 's/%//')
[ $DEBUG -eq 1 ] && echo "DEBUG: hd -> "$HD
HDUSED=$(df -hl /dev/vda2 | grep vda2 | awk '{print $3}' | sed -e 's/G//')
HDSIZE=$(df -hl /dev/vda2 | grep vda2 | awk '{print $2}' | sed -e 's/G//')
[ $DEBUG -eq 1 ] && echo "DEBUG: hd size -> $HDSIZE GB drive capacity"
TOPHD=$(df -hl /dev/vda2 | tail -1)
...
<tool>-=CPU $CPULOAD=-
<tt><small>$TOPCPU</small></tt>

-=MEM: $MEMUSED of $MEMUSAGE2=-
<tt><small>$TOPMEM</small></tt>

-=HD usage: $HDUSED of $HDSIZE GB in use=-
<tt><small>   $TOPHD</small></tt></tool>"
